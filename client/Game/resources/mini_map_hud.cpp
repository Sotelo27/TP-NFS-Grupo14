...
MiniMap::MiniMap(size_t client_id, const SdlWindow& window, const MapsTextures& map_manager,
        ...
        texture_cars_mini_map(CARS_MINI_MAP, window, Rgb(BACKGROUND_COLOR_R, BACKGROUND_COLOR_G,
                                                         BACKGROUND_COLOR_B)),
        draw_fill(window),
        client_id(client_id),
...
void MiniMap::renderCarOnMiniMap(int x_dest_mini_map, int y_dest_mini_map, int mini_map_width,
    ...
    int size_car_point_height = static_cast<float>(size_car_point_width) * HEIGHT_CAR_TEXTURE / WIDTH_CAR_TEXTURE;  // Keep aspect ratio
    ...
    texture_cars_mini_map.renderEntity(red ? AREA_RED_CAR_MINI_MAP : AREA_BLUE_CAR_MINI_MAP, car_dest_area_mini_map, info_car.angle);
}

...
void MonitorLobby::handle_cheat_action(ClientAction act) {
    ...
    uint8_t rid = binding->first;
    size_t pid = binding->second;
    // Enviar cualquier cheat al RoomManager/Game
    if (!rooms.push_cheat_to_room(rid, pid, act.cheat)) {
        std::cout << "[Lobby] CHEAT routing failed for conn_id=" << act.id
                  << " room_id=" << (int)rid << " cheat=" << (int)act.cheat << "\n";
    } else {
        std::cout << "[Lobby] CHEAT: Routed cheat=" << (int)act.cheat
                  << " to player_id=" << pid << " (conn_id=" << act.id << ")\n";
    }
}
